 ...
 * 
 * POST /api/search
 * Body: { skills: [], keywords: [], location: string, experience_level: string, job_categories: [] }
 * 
 * Searches multiple job platforms and returns matched results + direct search links
 */

const axios = require('axios');

// Optional: set your Jooble API key in Vercel env vars
...
async function searchRemotive(query, location) {
    ...
    try {
        ...
        for (const cat of categories) {
            ...
            const resp = await axios.get(url, { timeout: 15000 });
            for (const item of (resp.data.jobs || []).slice(0, 20)) {
                jobs.push({
                    title: item.title || '',
                    company: item.company_name || '',
...
async function searchRemoteOK(query, location) {
    ...
    try {
        const q = encodeURIComponent(query);
        const resp = await axios.get(`https://remoteok.com/remote-software-dev-jobs?page=1&search=${q}`, { timeout: 15000 });
        for (const item of resp.data.slice(0, 50)) {
            jobs.push({
                title: item.title || '',
...
async function searchJooble(query, location) {
    ...
    const jobs = [];
    try {
        const resp = await axios.post(`https://jooble.org/api/${JOOBLE_API_KEY}`, {
            keywords: query,
            location: location,
        }, { timeout: 15000 });
        for (const item of (resp.data.jobs || []).slice(0, 20)) {
...
async function searchHimalayas(query, location) {
    ...
    try {
        const q = encodeURIComponent(query);
        const resp = await axios.get(`https://himalayas.app/api/v1/jobs?q=${q}`, { timeout: 15000 });
        for (const item of (resp.data.results || []).slice(0, 50)) {
            jobs.push({
                title: item.title || '',
                company: item.company || '',
...
async function searchWeWorkRemotely(query, location) {
    ...
    try {
        for (const feedUrl of feeds) {
            ...
            const doc = parser.parseStringSync(xml);

            for (const item of (doc.rss.channel[0].item || [])) {
                const title = (item.title?.[0] || '').trim();
                const link = (item.link?.[0] || '').trim();
                const desc = stripHtml(item.description?.[0] || '');
...
/**
 * Generate direct search links for platforms that require login
 */
...
/**
 * API handler for Vercel/Netlify serverless functions
 */
...
if (require.main === module) {
    const testParams = {
        ...
        keywords: ['analytics', 'ML'],
        location: 'Hyderabad',
        experience_level: 'fresher',
        job_categories: ['india', 'software', 'ai_ml'],
    };
    searchJobs(testParams).then(console.log).catch(console.error);
}